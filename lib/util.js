...
function insertMiddleware(server, index, middlewares) {
  ...
  var stackCount = stackHolder.stack ? stackHolder.stack.length : 0;
...
exports.runServer = function runServer(grunt, options) {
  ...
  // "lr" needs to come before other middlwares being added because, if server
  // has not middleware stack, the index in insertMiddleware is ignored.
